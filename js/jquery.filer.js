...
(function($) {
    ...
    $.fn.filer.defaults = {
        ...
        captions: {
            ...
            removeConfirmation: "确定要删除文件吗?",
            errors: {
                filesLimit: "最大文件上传数为 {{fi-limit}}.",
                filesType: "只允许上传图片.",
                filesSize: "{{fi-name}} 文件大小超过 {{fi-fileMaxSize}} MB.",
                filesSizeAll: "总文件大小超过 {{fi-maxSize}} MB.",
...
var itemAppend = '<li class="jFiler-item">\
							<div class="jFiler-item-container">\
								<div class="jFiler-item-inner">\
									<div class="jFiler-item-thumb">\
										<div class="jFiler-item-status"></div>\
...
function successTemplete(data, itemEl, listEl, boxEl, newInputEl, inputEl, id) {
  var parent = itemEl.find(".jFiler-jProgressBar").parent(),
    filerKit = inputEl.prop("jFiler");
  //更新当前已上传完成的 file id 用于删除
  if (data.status == 0) {
    filerKit.files_list[id].file = data.data;
    itemEl.find(".jFiler-jProgressBar").fadeOut("slow", function () {
      $("<span class=\"jFiler-item-others ui green text\"><i class=\"ui check icon\"></i>上传成功</span>").hide().appendTo(parent).fadeIn("slow");
    });
  } else {
    el.find(".jFiler-jProgressBar").fadeOut("slow", function () {
      $("<span class=\"jFiler-item-others ui red text\"><i class=\"ui exclamation circle icon\"></i>上传出错</span>").hide().appendTo(parent).fadeIn("slow");
    });
  }
}

function errorTemplete (el) {
...
function deleteTemplete(itemEl, file, id, listEl, boxEl, newInputEl, inputEl) {
  var filerKit = inputEl.prop("jFiler"), fid = filerKit.files_list[id].file.id;
  if (fid) {
    $.ajax({
      type: 'DELETE',
      url: '/attachment/' + fid,
      success: function (data) {
        toast(data.status == 0 ? 'success' : 'error', data.msg)
      }
    })
  }
}